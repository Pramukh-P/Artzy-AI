...
const ShareModal = ({ postId, prompt, photo, onClose }) => {
  ...
  const shareText = `Check out this AI-generated image on Artzy-AI: "${prompt.slice(0, 60)}${prompt.length > 60 ? '…' : ''}"`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          ...
          >
            <div className="w-10 h-10 rounded-full bg-black dark:bg-gray-900 flex items-center justify-center shadow-lg">
              <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex gap-2">
          <input
            readOnly
            value={shareUrl}
            className="input-base text-xs flex-1 cursor-text"
          />
          <button
            onClick={copyLink}
            className={`shrink-0 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${
              copied
                ...
                : 'bg-[#6469ff] hover:bg-[#4f54d6] text-white'
            }`}
          >
            {copied ? '✓ Copied' : 'Copy'}
          </button>
